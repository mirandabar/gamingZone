...
void GameManager::updateGame() {
    ...
    for (auto& ball : m_balls) {
        ...
        if (collision) {
            Logger::info(FILE_NAME, "GameManager::updateGame", "Boundary collision detected for ball " + 
    ...
    for (size_t i = 0; i < m_balls.size(); ++i) {
        for (size_t j = i + 1; j < m_balls.size(); ++j) {
            if (m_collisionManager.checkCollision(m_balls[i], m_balls[j])) {
                Logger::info(FILE_NAME, "GameManager::updateGame", "Ball collision detected between ball " + 
                            std::to_string(m_balls[i].getId()) + " and ball " + std::to_string(m_balls[j].getId()));
